...
const std::unordered_map<std::string, TokenType> Lexer::keywords_ = {
    {"auto", TokenType::AUTO}, {"func", TokenType::FUNC}, {"return", TokenType::RETURN},
    {"if", TokenType::IF}, {"else", TokenType::ELSE}, {"for", TokenType::FOR}, {"in", TokenType::IN},
    {"class", TokenType::CLASS}, {"method", TokenType::METHOD}, {"operator", TokenType::OPERATOR},
    {"this", TokenType::THIS}, {"Private", TokenType::PRIVATE}, {"import", TokenType::IMPORT},
    {"as", TokenType::AS}, {"true", TokenType::TRUE_LITERAL}, {"false", TokenType::FALSE_LITERAL},
...
void Lexer::scan_token() {
    ...
    switch (c) {
        ...
        case ':': add_token(TokenType::COLON); break; // Xaq 类型标注
        ...
        case '-': 
            if (match('>')) add_token(TokenType::ARROW); // 处理函数返回符 ->
        ...
        case '<': 
            if (match('=')) add_token(TokenType::LESS_EQUAL);
                ...
                if (match('=')) add_token(TokenType::LESS_LESS_EQUAL);
                else add_token(TokenType::LESS_LESS);
            }
        ...
        case '>':
            if (match('=')) add_token(TokenType::GREATER_EQUAL);
                ...
                if (match('=')) add_token(TokenType::GREATER_GREATER_EQUAL);
                else add_token(TokenType::GREATER_GREATER);
            }
        ...
        case '*':
            if (match('*')) {
                if (match('=')) add_token(TokenType::STAR_STAR_EQUAL);
                else add_token(TokenType::STAR_STAR);
            } else {
...
void Lexer::doc_string() {
    ...
    (current_ + 2 < source_.length() && source_[current_ + 2] == '"')) && !is_at_end()) {
        if (peek() == '\n') line_++;
        advance();
    }
